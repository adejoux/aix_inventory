class AixWwpnsController < ApplicationController
  def index
    ...
    @search = Wwpn.server_type("VIO").includes(:san_infra).includes(:aix_port).customer_scope(current_user.customer_scope).search(session[:last_query])
    @total_records = Wwpn.server_type("VIO").customer_scope(current_user.customer_scope).count
