class Wwpn < ActiveRecord::Base
  ...
  def self.datatable_query
  	...
  	.joins('LEFT OUTER JOIN san_infras ON san_infras.id = wwpns.san_infra_id').select('wwpns.wwpn, servers.customer as customer, servers.hostname as hostname, san_infras.switch as switch,
  	                                         san_infras.portname as portname, san_infras.port as port ')
  end
end
